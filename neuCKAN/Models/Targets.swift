...
struct Targets: Hashable {
	...
	///
	///	Because of [a problem with generic default parameters][generic default parameters problem], use `init(groupVersion: CKANMetadataVersion, groupingLevel: GroupingLevel, conflictHandlingScheme: ConflictHandlingOption)` to initialise an empty collection of targets.
	///
	///	- Parameters:
	...
	init<T: Sequence>(targets: T, groupVersion: CKANMetadataVersion = CKANMetadataVersion(""), groupingLevel: GroupingLevel = .none, conflictHandlingScheme: ConflictHandlingOption = .preserveFirstOccurence) where T.Element == TargetConvertible? {
	...
	///
	///	[generic default parameters problem]: https://stackoverflow.com/questions/38326992/default-parameter-as-generic-type
	init(groupVersion: CKANMetadataVersion = CKANMetadataVersion(""), groupingLevel: GroupingLevel = .none, conflictHandlingScheme: ConflictHandlingOption = .preserveFirstOccurence) {
	...
	var majorVersionGroups: [CKANMetadataVersion: Targets] {
		Set<CKANMetadataVersion>(targets.map { CKANMetadataVersion($0.version[..<1]) } ).reduce(into: [:]) { groups, version in
	...
	var minorVersionGroups: [CKANMetadataVersion: Targets] {
		Set<CKANMetadataVersion>(targets.map { CKANMetadataVersion($0.version[..<2]) } ).reduce(into: [:]) { groups, version in
			groups[version] = Targets(
	...
	var patchVersionGroups: [CKANMetadataVersion: Targets] {
		Set<CKANMetadataVersion>(targets.map { CKANMetadataVersion($0.version[..<3]) } ).reduce(into: [:]) { groups, version in
			groups[version] = Targets(
	...
	let groupVersion: CKANMetadataVersion
