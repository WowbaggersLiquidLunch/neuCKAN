...
struct OrdinalVersion: Hashable {
	...
	/**
	Initialises a `OrdinalVersion` instance from the given version string.
	
	- Parameter versionString: The version string as defined by the CKAN metadata specification.
	*/
	init(_ versionString: String) {
		let finalVersionString = versionString == "any" ? OrdinalVersion.universallyCompatibleVersionString : versionString
		self.originalString = finalVersionString
		let deconstructedVersion = OrdinalVersion.deconstruct(from: finalVersionString)
	...
	/**
	Initialises a `OrdinalVersion` instance from the given version number.
	...
	*/
	fileprivate enum CKANVersionMinimalComparableUnit: Hashable, Comparable {
		...
		static func < (lhs: OrdinalVersion.CKANVersionMinimalComparableUnit, rhs: OrdinalVersion.CKANVersionMinimalComparableUnit) -> Bool {
			switch (lhs, rhs) {
	...
	*/
	init(from decoder: Decoder) throws {
		if let versionString = try? decoder.singleValueContainer().decode(String.self) {
			self = OrdinalVersion(versionString)
		} else if let versionDigits = try? decoder.singleValueContainer().decode(Double.self) {
			self = OrdinalVersion(versionDigits)
		} else {
			self = OrdinalVersion(String.defaultInstance)
		}
	}
...
fileprivate extension Array where Element == OrdinalVersion.CKANVersionMinimalComparableUnit {
	///	A textual representation of the version segment.
	var description: String { self.map { String(describing: $0) }.joined() }
}

extension OrdinalVersion.CKANVersionMinimalComparableUnit: CustomStringConvertible {
