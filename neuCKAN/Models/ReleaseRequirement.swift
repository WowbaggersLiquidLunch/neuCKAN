...
/**
The mod's relationship to another mod.

This is equivalent to an instance in the ["Relationship" type][0] in a CKAN metadata file.

`ReleaseRequirement` instances are building blocks for the relationship fields in `Release` instances. The relationship fields can be used to ensure that a mod is installed with one of its graphics packs, or two mods which conflicting functionality are not installed at the same time. The `ReleaseRequirement` struct translate a single instance in CKAN metadata's' `"Relationship"` field, following the same principle.

- Note: It is an error to mix `version` (which specifies an exact version) with either `min_version` or `max_version` in the same instance in a CKAN metadatas file.

[0]: https://github.com/KSP-CKAN/CKAN/blob/master/Spec.md#relationships
*/
struct ReleaseRequirement: Hashable, Codable {
...
extension ReleaseRequirement: CustomStringConvertible {
	/**
